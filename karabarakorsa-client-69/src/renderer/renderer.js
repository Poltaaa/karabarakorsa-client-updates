...
function snapChatToBottom(box) {
  ...
  // Senkron uygula: minimize/arka plan iken requestAnimationFrame tetiklenmez
  // ve "en alta yapisma" kaybolur. Bu yuzden once aninda kaydirilir (smooth
  // animasyon donmasin diye gecici 'auto'), ardindan rAF ile teyit edilir.
  const set = () => {
    if (!box.isConnected) return;
    const t = box.scrollHeight;
    if (box.scrollTop === t) return;
  ...
  set();
  if (typeof window.requestAnimationFrame === 'function') window.requestAnimationFrame(set);
...
// Pencere gizliyken rAF islemeyebilir; tekrar gorunur olunca en altta
// takip eden kutularin yapismasini teyit et (bilerek yukari kaydirdiysa
// kullaniciya dokunulmaz).
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState !== 'visible') return;
  const boxes = [logBox, chatBox, document.getElementById('dash-chat')];
  for (const b of boxes) {
    if (!b) continue;
    try {
      if (b.scrollTop + b.clientHeight >= b.scrollHeight - 120) snapChatToBottom(b);
    } catch (_) {}
  }
});
